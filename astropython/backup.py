        ...
        try:
            with open(address,"w") as outfile:
                if (model==Tutorial or model==Snippet or model==Wiki or model==Announcement or model==News or model==Blog):
                    json.dump({'title':obj.title,'slug':obj.slug,'abstract':obj.abstract,'input_type':obj.input_type,'body':(obj.body).encode('utf-8').strip(),'author':[user.username for user in obj.authors.all()],'state':obj.state,'published':obj.published,'created':str(obj.created),'tags':[tag_name.name for tag_name in obj.tags.all()]},outfile)
                elif model== Package:
                    json.dump({'title':obj.title,'slug':obj.slug,'abstract':obj.abstract,'input_type':obj.input_type,'body':(obj.body).encode('utf-8').strip(),'author':[user.username for user in obj.authors.all()],'state':obj.state,'published':obj.published,'created':str(obj.created),'tags':[tag_name.name for tag_name in obj.tags.all()],'homepage':obj.homepage,'docs':obj.docs},outfile)
